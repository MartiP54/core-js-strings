...
function removeFirstOccurrences(str, value) {
  ...
  return start === -1 ? str : str.slice(0, start) + str.slice(end);
}
...
function sumOfCodes(str) {
  return typeof str === 'string' || str instanceof String
    ? str
        .split('')
        .reduce((sum, number) => sum + Number(number.charCodeAt()), 0)
    : 0;
}
...
function isPalindrome(str) {
  ...
  const poly = str
    ...
    .toLowerCase()
    .replaceAll(regex, '');
  return str.toLowerCase().replaceAll(regex, '') === poly;
}
...
function findLongestWord(sentence) {
  ...
  result.map(function s(num, index) {
...
function invertCase(str) {
  return str
    .split('')
    .map((num) =>
      num === num.toLowerCase() ? num.toUpperCase() : num.toLowerCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  const regex = /[a-z]/gi;
  return str.replace(regex, (code) => output[input.indexOf(code)]);
}
